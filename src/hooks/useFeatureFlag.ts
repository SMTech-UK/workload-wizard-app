      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err
            : new Error("Failed to fetch feature flag"),
        );
        // Set a safe fallback
...
export function useFeatureFlags(
  ...
  const [results, setResults] = useState<
    Record<FeatureFlags, FeatureFlagResult>
  >({} as Record<FeatureFlags, FeatureFlagResult>);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<Error | null>(null);
...
export function useAllFeatureFlags(
  ...
  const allEnabled =
    flagNames.length > 0 && Object.values(flags).every((flag) => flag.enabled);
