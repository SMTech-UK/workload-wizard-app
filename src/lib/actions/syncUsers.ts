  ...
  try {
    ...
    for (const clerkUser of clerkUsers) {
      try {
        ...
        if (!primaryEmail) {
          console.warn(`Clerk user ${clerkUser.id} has no primary email address. Skipping.`);
          results.push({ userId: clerkUser.id, status: 'failed', message: 'No primary email address found' });
          continue;
        }
        ...
        if (!existingConvexUser) {
          ...
          results.push({ userId: clerkUser.id, status: 'created', message: 'User created in Convex' });
        } else {
          // User already exists in Convex
          results.push({ userId: clerkUser.id, status: 'skipped', message: 'User already exists in Convex' });
        }
      } catch (innerError) {
        console.error(`Error processing Clerk user ${clerkUser.id}:`, innerError);
        const errorMessage = innerError instanceof Error ? innerError.message : String(innerError);
        results.push({ userId: clerkUser.id, status: 'failed', message: `Failed to process: ${errorMessage}` });
      }
    }
...
export async function getSyncStatus() {
  ...
  try {
    ...
    const extraInConvex = convexUsers.filter(convexUser => !clerkUserIds.has(convexUser.subject));
