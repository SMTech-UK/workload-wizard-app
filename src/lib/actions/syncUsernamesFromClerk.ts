  ...
  try {
    ...
    for (const clerkUser of allClerkUsers) {
      try {
        ...
        if (!convexUser) {
          console.warn(
            `Clerk user ${clerkUser.id} not found in Convex. Skipping.`,
          );
          results.push({
            userId: clerkUser.id,
        ...
        console.log(
          `Updated username for user ${clerkUser.id}: "${convexUsername}" -> "${clerkUsername}"`,
        );
        results.push({
          userId: clerkUser.id,
          email: clerkUser.emailAddresses[0]?.emailAddress || "unknown",
    ...
    console.log(
      `Username sync completed: ${updatedCount} updated, ${skippedCount} skipped, ${errorCount} errors`,
    );
