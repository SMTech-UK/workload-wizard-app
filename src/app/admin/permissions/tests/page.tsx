...
type TestSummary = { totalTests: number; passedTests: number; failedTests: number };
type TestResultItem = { permission: string; expected: boolean; actual: boolean; passed: boolean };
type UserTestResult = { userId: string; results: TestResultItem[]; allPassed: boolean };
...
export default function PermissionTestsPage() {
  ...
  const runTests = useQuery(api.permissions.runPermissionTests, { nonce }) as TestRun | undefined;
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      ...
      <Button variant="outline" size="sm" onClick={handleCreateTestOrg} disabled={orgLoading}>
        {orgLoading ? 'Creating Org…' : 'Create Test Org'}
      </Button>
      <Button variant="outline" size="sm" onClick={handleCreateTestUsers} disabled={userLoading}>
        {userLoading ? 'Creating Users…' : 'Create Test Users'}
      </Button>
      <Button variant="outline" size="sm" onClick={handleSetupTestData} disabled={isLoading}>
        ...
        <CardContent className="space-y-6">
          ...
          {((testResults || runTests) as TestRun | null) && (
            <div className="space-y-4">
              <div className="flex gap-2 items-center">
                ...
                <Badge variant={((testResults || runTests) as TestRun).summary.failedTests === 0 ? "default" : "destructive"}>
                  {((testResults || runTests) as TestRun).summary.passedTests}/{((testResults || runTests) as TestRun).summary.totalTests} passed
                </Badge>
              </div>

              {((testResults || runTests) as TestRun).testResults.map((userTest: UserTestResult, index: number) => (
                <Card key={index}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      {userTest.results.map((result: TestResultItem, resultIndex: number) => (
          ...
          <div className="mt-8 p-4 bg-blue-50 rounded-lg">
            ...
            <ul className="space-y-1 text-sm">
              <li>Creates a TEST org, then roles from your current Default Role Templates (or Admin/Manager/Lecturer/Viewer fallback).</li>
              <li>Assigns permissions per `system_permissions.defaultRoles` mapping.</li>
              <li>Creates one test user per role (subject: <code>{'test_{role}_user'}</code>) plus a <code>test_sysadmin_user</code>.</li>
            </ul>
          </div>

          <div className="mt-4 p-4 bg-green-50 rounded-lg">
            <h4 className="font-semibold mb-2">Expected:</h4>
            <p className="text-sm">For each role, a permission passes if it is explicitly on the role or the role name is included in that permission’s <code>defaultRoles</code>. Sysadmin passes all.</p>
          </div>
            ...
            <div className="mt-4 p-4 bg-yellow-50 rounded-lg">
              ...
              {debugData.length === 0 ? (
                ...
                <div className="space-y-3">
                  {debugData.map((orgData: { org: { name: string; code: string }; roles: { name: string; permissions: string[] }[] }, index: number) => (
                    <div key={index} className="border-l-2 border-yellow-300 pl-3">
                      ...
                      <div className="text-sm text-gray-600">
                        {orgData.roles.length === 0 ? (
                          ...
                          <ul className="ml-4">
                            {orgData.roles.map((role: { name: string; permissions: string[] }, roleIndex: number) => (
                              <li key={roleIndex}>
