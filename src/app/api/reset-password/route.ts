  ...
  const isAdmin = userRole === 'sysadmin' || userRole === 'developer' || 
    ...
    if (isOrgAdmin) {
      ...
      if (targetUserOrgId !== currentUserOrgId) {
        return NextResponse.json(
          { error: 'Unauthorized: Can only reset passwords for users in your own organisation' },
          { status: 403 }
        );
      }
    }
    ...
    try {
      ...
      return NextResponse.json(
        { 
          ...
          nextSteps: 'Inform the user to visit the sign-in page and click "Forgot Password?" to reset their password.'
        },
        { status: 200 }
      );
