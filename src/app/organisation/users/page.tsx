...
export default function OrganisationUsersPage() {
  ...
  // Sorting
  type SortField = 'name' | 'email' | 'username' | 'role' | 'status' | 'created' | 'lastSignIn';
  ...
  const sortUsers = (list: User[], field: SortField, direction: SortDirection) => {
    return [...list].sort((a, b) => {
      let aVal: string | number;
      let bVal: string | number;
      switch (field) {
        case 'name':
          aVal = `${a.givenName || ''} ${a.familyName || ''}`.toLowerCase().trim();
          bVal = `${b.givenName || ''} ${b.familyName || ''}`.toLowerCase().trim();
          break;
        case 'email':
  ...
  const toggleSelectAll = () => {
    if (selectedUserIds.size === sortedUsers.length) setSelectedUserIds(new Set());
  ...
  const openAssignRoles = (target: User | null, bulk = false) => {
    ...
    const preselectedOrgRoleIds = !bulk
      ? (((target as unknown) as { organisationalRoles?: { id: string }[] })?.organisationalRoles || []).map((r) => r.id)
      : [];
        ...
        const updates = targets.map(u => (
          fetch('/api/update-user', {
            ...
            body: JSON.stringify({
              ...
              organisationalRoleIds: selectedOrgRoleIds.length ? selectedOrgRoleIds : undefined,
                ...
                <DropdownMenuContent align="start" className="w-80 p-4">
                  <div className="space-y-4">
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="text-sm font-medium mb-2 block">Organisation Role</label>
                    ...
                    <div className="flex justify-end pt-2 border-t">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => { setSearchTerm(''); setRoleFilter('all'); setOrgRoleFilter('all'); setStatusFilter('all'); }}
                      >
                        Clear All
                      </Button>
                    </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50 border-b-2 border-muted-foreground/20">
                  ...
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[18%]" onClick={() => handleSort('name')}>
                    <div className="flex items-center justify-center gap-1">Name {getSortIcon('name')}</div>
                  </TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[18%]" onClick={() => handleSort('email')}>
                    <div className="flex items-center justify-center gap-1">Email {getSortIcon('email')}</div>
                  </TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[12%]" onClick={() => handleSort('username')}>
                    <div className="flex items-center justify-center gap-1">Username {getSortIcon('username')}</div>
                  </TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[16%]" onClick={() => handleSort('role')}>
                    <div className="flex items-center justify-center gap-1">System Roles {getSortIcon('role')}</div>
                  </TableHead>
                  <TableHead className="text-center font-semibold text-sm py-4 w-[16%]">Organisation Role</TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[10%]" onClick={() => handleSort('status')}>
                    <div className="flex items-center justify-center gap-1">Status {getSortIcon('status')}</div>
                  </TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[12%]" onClick={() => handleSort('created')}>
                    <div className="flex items-center justify-center gap-1">Created {getSortIcon('created')}</div>
                  </TableHead>
                  <TableHead className="text-center cursor-pointer hover:bg-muted transition-colors font-semibold text-sm py-4 w-[12%]" onClick={() => handleSort('lastSignIn')}>
                    <div className="flex items-center justify-center gap-1">Last Sign In {getSortIcon('lastSignIn')}</div>
                  </TableHead>
                ...
                {sortedUsers.map((user) => (
                  <TableRow key={user._id}>
                    <TableCell className="text-center">
                      <input type="checkbox" checked={selectedUserIds.has(user._id)} onChange={() => toggleSelectUser(user._id)} />
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-center">
                      <div className="flex flex-wrap gap-1 justify-center">
                        {(user.systemRoles || []).length === 0 ? (
                          <span className={`px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800`}>None</span>
                        ) : (
                          user.systemRoles.map((r) => (
                            <span key={r} className={`px-2 py-1 rounded-full text-xs font-medium ${getRoleBadgeClass(r)}`}>
                              {getRoleLabel(r)}
                            </span>
                          ))
                        )}
                    ...
                    <TableCell className="text-center">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                          <DropdownMenuItem onClick={() => openAssignRoles(user, false)}>
                  ...
                  {(orgRoles || []).map((r: any) => {
                    ...
                    return (
                      <button
                        key={r._id}
                        type="button"
                        onClick={() => setSelectedOrgRoleIds(checked ? selectedOrgRoleIds.filter(id => id !== r._id) : [...selectedOrgRoleIds, r._id])}
                        className={`px-2 py-1 rounded border text-xs ${checked ? 'bg-slate-900 text-white' : 'bg-white'}`}
                      >
                        {r.name}
                      </button>
