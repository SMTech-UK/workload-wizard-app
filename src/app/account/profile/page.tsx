...
import { Settings, User, Shield, Mail, Calendar, Camera, Save, X, RefreshCw } from "lucide-react"
...
export default function ProfilePage() {
  ...
  const convexAvatarUrl = useQuery(api.users.getUserAvatar, 
  ...
  const avatarUrl = clerkAvatarUrl ? 
    ...
    (convexAvatarUrlValue ? `${convexAvatarUrlValue}?r=${avatarRefreshKey}` : "")
        ...
        if (avatarFile && avatarFile.size > 0) {
          try {
            ...
            try {
              ...
              throw new Error(`Clerk upload failed: ${clerkError instanceof Error ? clerkError.message : 'Unknown error'}`)
            }
            ...
            if (updatedImageUrl && updatedImageUrl !== avatarUrl) {
              ...
              toast({
                title: "Avatar Update",
                description: "Avatar uploaded to Clerk. It may take a moment to appear.",
                variant: "success",
              })
            }
          } catch (avatarError) {
